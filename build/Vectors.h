...
struct Vector4
{
	...
	Vector4&    operator/=(const float scale);          // scale and update this object
	Vector4		quatMulply(const Vector4 rhs);			//quaternion multiplication
...
inline Vector4 Vector4::quatMulply(const Vector4 rhs)
{
	Vector3 v1 = Vector3(x, y, z);
	Vector3 v2 = Vector3(rhs.x, rhs.y, rhs.z);
	float x1 = x; float y1 = y; float z1 = z;
	float x2 = rhs.x; float y2 = rhs.y; float z2 = rhs.z;
